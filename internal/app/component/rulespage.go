...
func RulesPage() *hypp.VNode {
	return html.Main(
		...
		html.P(nil, replaceLinks("This page contains the rules of Senet. You can also learn the rules with the [interactive-tutorial-link].")...),
		html.Ul(
			nil,
		...
		html.P(nil, replaceIcons("Below you see the board and pieces. Player 1 plays with the blue squares [piece-0-icon]. Player 2 plays with the red circles [piece-1-icon]. Use a coinflip to decide who becomes player 1. Player 1 goes first.")...),
		Board(&state.State{
			Game: state.NewGame(),
		}),
